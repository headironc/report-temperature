...
let log = fs.createWriteStream(
  `${__dirname}/logs/${getFormatDate()}.log`,
  options
);
...
class ReportTemperature {
  ...
  async login(index) {
    const res = await axios.post(
      "https://yqfk.mydongtai.cn:20443/appintf/login/loginUser.intf",
      {
    ...
    logger.log(
      `${index}-${res?.data?.code} ${res?.data?.msg}---${
        this.name
      }---login--${getFormatDate()}`
    );
  ...
  async getMsAndFamilyInfo(index) {
    ...
    let res = await axios.get(
      "https://yqfk.mydongtai.cn:20443/appintf/tere/getMSAndFamilyInfo",
      {
    ...
    logger.log(
      `${index}-${res?.data?.code} ${res?.data?.msg}---${
        this.name
      }---getMsAndFamilyInfo--${getFormatDate()}`
    );
    ...
    if (res?.data?.code === 401) {
      ...
      res = await axios.get(
        "https://yqfk.mydongtai.cn:20443/appintf/tere/temperReport.intf",
        {
          headers: headers(this.token),
        }
      );
      ...
      logger.log(
        `${index}-${res?.data?.code} ${res?.data?.msg}---${
          this.name
        }---getMsAndFamilyInfo--when-failed--${getFormatDate()}`
      );
    }
  ...
  async report(index) {
    ...
    logger.log(
      `${index}-${res?.data?.code} ${res?.data?.msg}---${
        this.name
      }--report-temperature--${getFormatDate()}-${this.temperature}°C`
    );
...
const getCredentials = async () => {
  return JSON.parse(
    (await readFile(`${__dirname}/info.json`, "utf8")).toString()
  );
};
...
const operate = async () => {
  ...
  const toWrited = {};
  for (const [index, { username, password, token, name }] of Object.values(
    res
  ).entries()) {
